...
import { calculateTDH, calculateBaseHead, findIntersection, calculateSystemResults, calculateBasePowerPerStage, calculateMotorPoly, generateMultiCurveData, getShaftLimitHp, calculatePDP, calculateFluidProperties, interpolateTVD, calculatePwf, getDownloadFilename } from '../utils';
...
export const DesignReport = ({ onClose, params, pump, results, frequency, motor, cable, type = 'target' }: any) => {
    ...
    const rowDefinitions = [
        ...
        { label: t('sens.motorHp'), key: 'motorHp', unit: 'hp', fmt: f0 },
    ...
    return (
        <div className="fixed inset-0 z-[9999] bg-canvas overflow-hidden flex flex-col animate-fadeIn report-outer-container">
            ...
            <div className="flex-1 overflow-y-auto bg-slate-100 p-8 custom-scrollbar report-outer-container">
                <style>{`
                    @media print {
                        ...
                        /* Ensure the report and all its parents are visible and not clipped */
                        #design-report-paper, 
                        #design-report-paper *,
                        ...
                        .page-break { page-break-before: always; display: block !important; clear: both; }
                        * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
                    }
                    /* Force Light Mode Styles for Report */
                    #design-report-paper {
                        background-color: white !important;
                        color: #0f172a !important; /* slate-900 */
                    }
                    #design-report-paper h1, #design-report-paper h2, #design-report-paper h3, #design-report-paper h4 { color: #0f172a !important; }
                    #design-report-paper p, #design-report-paper span, #design-report-paper td, #design-report-paper th { color: #334155 !important; } /* slate-700 */
                    #design-report-paper .text-white { color: white !important; }
                    #design-report-paper .bg-slate-900 { background-color: #0f172a !important; }
                    #design-report-paper .bg-slate-50 { background-color: #f8fafc !important; }
                    #design-report-paper .border-slate-900 { border-color: #0f172a !important; }
                    #design-report-paper .border-slate-200 { border-color: #e2e8f0 !important; }
                    #design-report-paper .text-slate-400 { color: #94a3b8 !important; }
                    #design-report-paper .text-slate-500 { color: #64748b !important; }
                    #design-report-paper .text-slate-900 { color: #0f172a !important; }
                    
                    .report-container { max-width: 210mm; margin: 0 auto; background: white; padding: 25px; box-shadow: 0 10px 50px rgba(0,0,0,0.1); border-radius: 8px; }
                    .chart-container-report { height: 380px !important; margin-bottom: 20px; }
                `}</style>
                ...
                <div id="design-report-paper" className="report-container">
                    ...
                    {scenariosToRender.map((scenarioId, sIdx) => {
                        ...
                        return (
                            <div key={scenarioId} className={(type === 'comprehensive' && sIdx >= 0) || (type !== 'comprehensive' && sIdx > 0) ? "page-break pt-10" : ""}>
                                    ...
                                    <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-center">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            {[
                                                ...
                                                { label: 'Motor Amps', val: sRes.electrical?.amps?.toFixed(1), unit: 'A' },
                                                { label: 'Power', val: sRes.electrical?.kw?.toFixed(1), unit: 'kW' },
                                                { label: 'Motor Load', val: sRes.motorLoad?.toFixed(1), unit: '%' },
                                                { label: 'Efficiency', val: sRes.effEstimated?.toFixed(1), unit: '%' },
                                                { label: 'Pwf', val: sRes.pwf?.toFixed(0), unit: 'PSI' },
                                            ].map((kpi, k) => (
...
export const Phase5: React.FC<Phase5Props> = ({ params, setParams, customPump, setCustomPump, pumpCatalog, motorCatalog, setShowPumpModal, curveData, match, results, onCompare }) => {
    const { t } = useLanguage();
    ...
    const effectivePump = useMemo(() => {
        ...
        return {
            ...customPump,
            h0: customPump.h0 * derating.head, h1: customPump.h1 * derating.head, h2: customPump.h2 * derating.head, h3: customPump.h3 * derating.head, h4: customPump.h4 * derating.head, h5: customPump.h5 * derating.head, h6: customPump.h6 * derating.head,
            p0: customPump.p0 * derating.power, p1: customPump.p1 * derating.power, p2: customPump.p2 * derating.power, p3: customPump.p3 * derating.power, p4: customPump.p4 * derating.power, p5: customPump.p5 * derating.power, p6: customPump.p6 * derating.power,
            maxEfficiency: customPump.maxEfficiency * derating.eff
        };
    }, [customPump, derating]);
    ...
    const [showViableOnly, setShowViableOnly] = useState(true);
    ...
    const rowDefinitions = [
        { label: "Frequency", key: 'hz', unit: 'Hz', fmt: (n: number) => n.toFixed(1) },
        { label: "BFPD", key: 'flow', unit: 'bpd', fmt: (n: number) => n.toFixed(0) },
        { label: "TDH", key: 'tdh', unit: 'ft', fmt: (n: number) => n.toFixed(0) },
        { label: "PIP", key: 'pip', unit: 'psi', fmt: (n: number) => n.toFixed(0) },
        { label: "Motor Amps", key: 'amps', unit: 'A', fmt: (n: number) => n.toFixed(1) },
        { label: "Motor Volts", key: 'volts', unit: 'V', fmt: (n: number) => n.toFixed(0) },
        { label: "Power", key: 'kw', unit: 'kW', fmt: (n: number) => n.toFixed(1) },
        { label: "Efficiency", key: 'pumpEff', unit: '%', fmt: (n: number) => n.toFixed(1) },
        { label: "Pump Shaft", key: 'pumpShaftLoad', unit: '%', fmt: (n: number) => n.toFixed(0) },
        { label: "Motor Load", key: 'motorShaftLoad', unit: '%', fmt: (n: number) => n.toFixed(0) },
        { label: "Motor Temp", key: 'motorT', unit: '°F', fmt: (n: number) => n.toFixed(0) },
    ];
    ...
    const switchScenario = (scen: 'min' | 'target' | 'max') => {
        ...
        setParams(prev => ({ ...prev, activeScenario: scen, pressures: { ...prev.pressures, totalRate: data.rate }, inflow: { ...prev.inflow, ip: data.ip }, fluids: { ...prev.fluids, waterCut: data.waterCut, gor: data.gor, glr: data.gor * (1 - data.waterCut / 100) } }));
    };

    // --- SMART CATALOG PROCESSING ---
    const activePumpList = useStandardPumps ? STANDARD_PUMPS : pumpCatalog;
    const activeMotorList = useStandardPumps ? STANDARD_MOTORS : motorCatalog;
    ...
    const groupedPumps = useMemo(() => {
        ...
        const processed = filtered.map(pump => {
            ...
            const isViable = targetFlow >= minLimit && targetFlow <= maxLimit;
    ...
    const processedMotors = useMemo(() => {
        let filtered = activeMotorList.filter(m => m && m.id);
        if (motorSearch) { const s = motorSearch.toLowerCase(); filtered = filtered.filter(m => m.model.toLowerCase().includes(s) || m.manufacturer.toLowerCase().includes(s) || m.hp.toString().includes(s)); }
    ...
    }, [activeMotorList, motorSearch, activeResults.requiredMotorHp]);
    ...
    return (
        <div className="h-full flex flex-col gap-6 animate-fadeIn pb-2 overflow-hidden px-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6 shrink-0">
                ...
                <KPICard label={t('kpi.power')} value={activeResults?.electrical?.kw?.toFixed(1) || '-'} unit="kW" icon={Cpu} colorClass="secondary" />
            ...
            <div className="flex-1 min-h-0 flex flex-col lg:flex-row gap-6 overflow-hidden pr-1">
                ...
                <div className="flex-1 lg:flex-[0.28] flex flex-col gap-6 overflow-y-auto custom-scrollbar min-h-0 pr-1">
                    ...
                    <CollapsibleSection title="ESP COMPONENTS" icon={Layers} isOpen={openSections.bhaComponents} onToggle={() => toggleSection('bhaComponents')} colorClass="primary">
                        <div className="p-4 space-y-4 bg-canvas/30 rounded-b-2xl">
                            {/* DB Switch */}
                            <div className="flex glass-surface p-1 rounded-2xl border border-white/5 shadow-inner relative overflow-hidden group">
                                <button onClick={() => setUseStandardPumps(true)} className={`flex-1 py-2.5 rounded-xl text-[9px] font-black uppercase transition-all relative z-10 ${useStandardPumps ? 'bg-primary/20 text-primary border border-primary/20 shadow-glow-primary/10' : 'text-txt-muted'}`}>Standard</button>
                                <button onClick={() => setUseStandardPumps(false)} className={`flex-1 py-2.5 rounded-xl text-[9px] font-black uppercase transition-all relative z-10 ${!useStandardPumps ? 'bg-secondary/20 text-secondary border border-secondary/20 shadow-glow-secondary/10' : 'text-txt-muted'}`}>Imported</button>
                            </div>
                            ...
                            <CollapsibleSection title={t('p5.motors')} icon={Zap} isOpen={openSections.motors} onToggle={() => toggleSection('motors')} colorClass="secondary">
                                <div className="p-3 space-y-3 bg-white/5">
                                    <div className="relative">
                                        <input type="text" value={motorSearch} onChange={(e) => setMotorSearch(e.target.value)} placeholder="Search Motor..." className="w-full bg-canvas/40 border border-white/5 rounded-xl py-2.5 pl-9 pr-4 text-[10px] font-black text-txt-main outline-none focus:border-secondary/40 uppercase" />
                                        <Search className="w-3.5 h-3.5 text-txt-muted absolute left-3 top-1/2 -translate-y-1/2" />
                                    </div>
                                    <div className="max-h-[250px] overflow-y-auto custom-scrollbar space-y-2 pr-1">
                                        {processedMotors.map(({ motor, isFit }, idx) => {
                                            ...
                                            return (
                                                <button key={idx} onClick={() => { handleMotorSelect(motor); setActiveChart('motor'); }} className={`w-full text-left p-3 rounded-xl border transition-all ${isActive ? 'bg-secondary/10 border-secondary' : isFit ? 'glass-surface-light border-white/5 hover:border-secondary/40' : 'bg-red-500/10 border-red-500/30 hover:border-red-500/50'}`}>
                                                    <div className="min-w-0 flex-1">
                                                        <div className={`text-[10px] font-black uppercase truncate ${isActive ? 'text-secondary' : 'text-txt-main'}`}>{motor.model}</div>
                                                        <div className="text-[7px] text-txt-muted font-black uppercase opacity-60">{motor.hp} HP</div>
                                                    </div>
                                                    <div className="text-right ml-2 text-[9px] font-black font-mono">{motor.voltage}V</div>
                                                </button>
                                            );
                                        })}
                                    </div>
                ...
                <div className="flex-1 lg:flex-[0.44] flex flex-col gap-6 overflow-y-auto custom-scrollbar min-h-0 pr-2 pb-4">
                    <div className="flex glass-surface p-1.5 rounded-[1.5rem] border border-white/5 shadow-inner shrink-0 relative overflow-hidden group">
                        ...
                        <button onClick={() => setActiveChart('pump')} className={`flex-1 py-3 rounded-2xl text-xs font-black uppercase flex items-center justify-center gap-3 transition-all relative z-10 ${activeChart === 'pump' ? 'bg-primary/20 text-primary shadow-glow-primary/10 border border-primary/20' : 'text-txt-muted hover:text-txt-main hover:bg-white/5'}`}>
                            <Activity className="w-4 h-4" /> {t('p5.perfCurve')}
                        </button>
                        <button onClick={() => setActiveChart('motor')} className={`flex-1 py-3 rounded-2xl text-xs font-black uppercase flex items-center justify-center gap-3 transition-all relative z-10 ${activeChart === 'motor' ? 'bg-secondary/20 text-secondary shadow-glow-secondary/10 border border-secondary/20' : 'text-txt-muted hover:text-txt-main hover:bg-white/5'}`}>
                            <Zap className="w-4 h-4" /> {t('p5.motorCurve')}
                        </button>
                    </div>
                    <div className="glass-surface-light rounded-[2.5rem] border border-white/5 shadow-2xl overflow-hidden p-3 relative flex flex-col shrink-0 group transition-all duration-700" style={{ height: '420px' }}>
                    ...
                    <div className="glass-surface rounded-[2rem] border border-white/5 p-4 flex gap-4 shadow-xl shrink-0 overflow-x-auto relative group light-sweep">
                        <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-primary rounded-full blur-[2px] opacity-0 group-hover:opacity-100 transition-all"></div>
                        <DigitalReadout label={t('p5.motorAmps')} value={activeResults?.electrical?.amps?.toFixed(1)} unit="A" color="amber" />
                        <DigitalReadout label={t('p5.motorVolts')} value={activeResults?.electrical?.volts?.toFixed(0)} unit="V" color="yellow" />
                        <DigitalReadout label="DEMANDA SISTEMA" value={activeResults?.electrical?.systemKva?.toFixed(1)} unit="kVA" color="purple" />
                        <DigitalReadout label="POT. SUPERFICIE" value={activeResults?.electrical?.systemKw?.toFixed(1)} unit="kW" color="blue" />
                        <DigitalReadout label={t('p5.cableLoss')} value={activeResults?.electrical?.voltDrop?.toFixed(0)} unit="V" color="red" />
                    </div>

                    {/* VSD TABLE */}
                    <CollapsibleSection title={t('p5.vsdAnalysis')} icon={Table} isOpen={openSections.vsdTable} onToggle={() => toggleSection('vsdTable')} colorClass="primary">
                        <div className="p-0 overflow-hidden min-h-[300px] glass-surface-light">
                            <table className="w-full text-[10px] text-left border-collapse">
                                <thead className="bg-primary/10 text-primary uppercase font-black sticky top-0 z-10 backdrop-blur-md border-b border-primary/20">
                                ...
                                <tbody className="divide-y divide-white/5">
                                    {vsdTableData.map((row, i) => {
                                        ...
                                        return (
                                            <tr key={i} className={`hover:bg-primary/5 transition-all duration-300 group ${isCurrent ? 'bg-primary/10' : ''}`}>
                                                {rowDefinitions.map(def => (
                                                    <td key={def.key} className={`py-4 px-4 text-center font-mono font-black ${isCurrent ? 'text-primary' : 'text-txt-main opacity-80 group-hover:opacity-100'} transition-all`}>
                                                        {def.fmt(row[def.key as keyof typeof row] as number)}
                                                    </td>
                                                ))}
                                            </tr>
                                        )
                            ...
                            <div className="p-4 flex justify-between items-center opacity-40">
                                <div className="text-[9px] font-black uppercase tracking-[0.3em]">Variable Speed Drive Dynamics @ SG {activeResults?.sgMixed?.toFixed(2) || '1.0'}</div>
                                <div className="flex gap-2"><div className="w-1 h-3 bg-primary/40 rounded-full"></div><div className="w-1 h-3 bg-secondary/40 rounded-full"></div></div>
                            </div>
                        </div>
                    </CollapsibleSection>
                </div>
                ...
                <div className="flex-1 lg:flex-[0.28] flex flex-col gap-6 min-h-0 overflow-y-auto custom-scrollbar pr-1">
                    ...
                    <div className="glass-surface p-2 rounded-[2rem] border border-white/5 shadow-xl flex items-center gap-2 shrink-0 relative overflow-hidden group">
                        <div className="absolute inset-0 bg-gradient-to-br from-primary/5 via-transparent to-secondary/5"></div>
                        {['min', 'target', 'max'].map((s) => (
                            <button
                                key={s}
                                onClick={() => switchScenario(s as any)}
                                className={`flex-1 py-5 rounded-[1.5rem] text-[10px] font-black uppercase transition-all duration-500 flex flex-col items-center justify-center gap-2 relative z-10 border group ${params.activeScenario === s ? 'bg-primary/20 text-primary border-primary/40 shadow-glow-primary/20' : 'glass-surface-light border-white/5 text-txt-muted hover:text-txt-main hover:border-white/10'}`}
                            >
                                <span className={`text-[9px] tracking-[0.2em] font-black ${params.activeScenario === s ? 'opacity-100' : 'opacity-40'}`}>{s === 'min' ? 'IDLE' : s === 'target' ? 'NOMINAL' : 'LIMIT'}</span>
                                <span className="text-sm font-mono tracking-tighter leading-none">{params.targets[s as keyof typeof params.targets].rate} <small className="opacity-60">BPD</small></span>
                                {params.activeScenario === s && <div className="absolute bottom-1 w-8 h-1 bg-primary rounded-full shadow-glow-primary"></div>}
                            </button>
                        ))}
                    </div>
