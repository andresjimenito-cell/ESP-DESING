...
export const PhaseSimulations: React.FC<Props> = ({ params, setParams, pump, frequency }) => {
    ...
    const simulationData = useMemo(() => {
        ...
        const targetRate = params.pressures.totalRate || 1000;

        for (let m = 0; m <= simulationMonths; m++) {
            const wearPct = (annualWearPercent / Math.max(1, simulationMonths)) * m;
            const headWearFactor = Math.max(0.1, 1 - (wearPct / 100));
            const effWearFactor = Math.max(0.1, 1 - ((wearPct * 1.2) / 100));
            ...
            // 1. Encontrar la frecuencia que mantendría el caudal objetivo (Equilibrio)
            let lowF = 30, highF = 95;
            for (let k = 0; k < 15; k++) {
                const midF = (lowF + highF) / 2;
                const hPump = calculateAffinityHead(targetRate, midF, baseFreq, degradedPump);
                const hSys = calculateTDH(targetRate, params);
                if (hPump > hSys) highF = midF; else lowF = midF;
            }
            const stableFreq = (lowF + highF) / 2;

            // 2. Aplicar Ramp-Up (Primeros 4 meses) para simular arranque (Unloading)
            let requiredFreq = stableFreq;
            if (m === 0) requiredFreq = stableFreq * 0.895;
            else if (m === 1) requiredFreq = stableFreq * 0.943;
            else if (m === 2) requiredFreq = stableFreq * 0.972;
            else if (m === 3) requiredFreq = stableFreq * 0.991;

            // 3. Calcular el caudal real resultante bajo esta frecuencia
            let currentFlow = targetRate;
            if (m < 4) {
                let lowQ = 0, highQ = targetRate * 1.6;
                for (let k = 0; k < 20; k++) {
            ...
            const mPerf = params.selectedMotor ? calculateMotorPoly(loadPct, params.selectedMotor) : { ampsPct: 0, pf: 0, rpm: 0, eff: 90 };
            ...
            timeData.push({
                month: m, wearPct, freq: requiredFreq, cumulativeCost: totalCumulativeCost, monthlyCost, flow: Math.round(currentFlow), head: Math.round(res.tdh || 0),
                efficiency: (res.effEstimated || 0) * effWearFactor, bhp: wornBhp, kw: wornKw, motorLoad: loadPct,
        ...
        return {
            timeData, startFlow: timeData[0]?.flow || 0, endFlow: timeData[timeData.length - 1]?.flow || 0,
            totalCost: totalCumulativeCost, startOP: timeData[0]?.op || { flow: 0, head: 0 },
            startLoad: timeData[0]?.motorLoad || 0, endLoad: timeData[timeData.length - 1]?.motorLoad || 0
        };
    ...
    const multiAxisData = useMemo(() => {
        ...
        for (let i = 0; i <= 60; i++) {
            ...
            if (hN >= 0) data.push({
                flow: Math.round(q), headNew: Number(hN.toFixed(1)), headCurr: Number(hC.toFixed(1)),
                effNew: Number(eN.toFixed(1)), effCurr: Number(eC.toFixed(1)),
                pwrNew: Number(pN.toFixed(1)), pwrCurr: Number(pC.toFixed(1))
            });
        }
        return data;
                ...
                <div className="col-span-12 lg:col-span-4 flex flex-col gap-6 overflow-y-auto custom-scrollbar pr-2 min-h-0">
                    ...
                    <div className="glass-surface-light border border-white/5 rounded-[2rem] p-6 shadow-xl relative overflow-hidden group">
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            ...
                            <PremiumParam label={t('sim.time')} value={params.simulation.simulationMonths} unit={t('sim.months')} onChange={(v) => setParams({ ...params, simulation: { ...params.simulation, simulationMonths: v } })} />
                        </div>
                    </div>
                </div>
