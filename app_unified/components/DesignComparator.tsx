...
import {
    ...
    Radar as RadarIcon, BarChart as LucideBarChart, Droplets
} from 'lucide-react';
import {
...
export const DesignComparator: React.FC<Props> = ({ onBack, initialDesign }) => {
    ...
    const { theme, toggleLightMode } = useTheme();
                ...
                <header className="sticky top-0 z-50 glass-surface px-4 h-14 flex items-center justify-between border-b border-surface-light/80 shadow-lg">
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className="bg-surface/30 hover:bg-surface border border-white/10 hover:border-surface-light rounded-full px-4 h-9 flex items-center gap-2 backdrop-blur-md transition-all group"
                        >
                            <Globe className="w-3.5 h-3.5 text-txt-muted group-hover:text-primary transition-colors" />
                            <span className="text-[8px] font-black uppercase tracking-[0.1em] text-txt-main">
                                {language === 'en' ? 'EN' : 'ES'}
                            </span>
                        </button>
                        ...
                        <button
                            ...
                            className="w-9 h-9 flex items-center justify-center bg-surface/30 hover:bg-surface border border-white/10 hover:border-surface-light rounded-full backdrop-blur-md transition-all group"
                        >
                            <span className="text-sm group-hover:scale-110 transition-transform">
                                {(theme === 'executive' || theme === 'heritage') ? '🌙' : '☀️'}
                            </span>
                        </button>
                    </div>
                </header>
